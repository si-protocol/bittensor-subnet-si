...
# Check if the current directory contains bittensor-subnet-si
if [ ! -d "bittensor-subnet-si" ]; then
    echo "Please run this script from the parent directory of bittensor-subnet-si."
    echo "The bittensor-subnet-si directory is not found in the current directory."
    exit 1
fi
...
setup_environment() {
    ...
    # Install the bittensor-subnet-si python package
    python -m pip install -e .

    # Create and set up wallets
    # This section can be skipped if wallets are already set up
    if [ ! -f ".wallets_setup" ]; then
        echo "please run btcli wallet new_coldkey --wallet.name $wallet to create a coldkey"
        echo "please run btcli wallet new_hotkey --wallet.name $wallet to create a hotkey"
        echo "please run btcli wallet new_coldkey --wallet.name miner to create a coldkey"
        echo "please run btcli wallet new_hotkey --wallet.name miner to create a hotkey"
        echo "please run btcli wallet new_coldkey --wallet.name validator to create a coldkey"
        echo "please run btcli wallet new_hotkey --wallet.name validator to create a hotkey"
        # touch .wallets_setup
    fi
...
## Setup localnet
# assumes we are in the bittensor-subnet-si/ directory
